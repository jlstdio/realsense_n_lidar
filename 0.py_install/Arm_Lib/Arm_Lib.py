...
class Arm_Device(object):
    ...
    def __init__(self):
        self.addr = 0x15
        self.bus_id = self._detect_i2c_bus()
        self.bus = smbus.SMBus(self.bus_id)

    @staticmethod
    def _detect_i2c_bus():
        env_bus = os.environ.get("ARM_I2C_BUS")
        if env_bus is not None:
            try:
                return int(env_bus)
            except ValueError as exc:
                raise ValueError("ARM_I2C_BUS must be an integer, e.g. 1 or 20") from exc

        # Keep the original default first for Raspberry Pi compatibility,
        # then fall back to any available /dev/i2c-* bus on this machine.
        candidates = [1, 0]
        for path in glob("/dev/i2c-*"):
            try:
        ...
        raise FileNotFoundError(
            "No I2C bus found. Expected /dev/i2c-1 or similar. "
            "Enable I2C and verify /dev/i2c-* exists."
        )
